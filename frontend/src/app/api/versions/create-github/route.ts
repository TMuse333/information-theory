...
export async function POST(req: NextRequest) {
  try {
    ...
    // Debug: Check what pages structure is in the received data
    const websiteDataFile = files.find(f => f.path.includes('websiteData.json'));
    if (websiteDataFile) {
      const parsedContent = JSON.parse(websiteDataFile.content) as any;

      // Extract title for debugging
      const firstPageTitle = Array.isArray(parsedContent.pages)
        ? parsedContent.pages[0]?.components?.[0]?.props?.title
        : (Object.values(parsedContent.pages || {})[0] as any)?.components?.[0]?.props?.title;

      console.log("📥 [create-github API] Received websiteData:", {
        pagesIsArray: Array.isArray(parsedContent.pages),
        pagesCount: Array.isArray(parsedContent.pages) ? parsedContent.pages.length : Object.keys(parsedContent.pages || {}).length,
        firstPageSlug: Array.isArray(parsedContent.pages) ? parsedContent.pages[0]?.slug : Object.keys(parsedContent.pages)[0],
    ...
    const commitsResponse = await fetch(
      `https://api.github.com/repos/${REPO_OWNER}/${REPO_NAME}/commits?sha=${branch}&per_page=100`,
      {
        headers: {
          Authorization: `token ${GITHUB_TOKEN}`,
          Accept: "application/vnd.github.v3+json",
    ...
    if (commitsResponse.ok) {
      ...
      console.log(`📊 [create-github API] Calculated version number: ${versionNumber} (from ${commits.length} commits)`);
    }
