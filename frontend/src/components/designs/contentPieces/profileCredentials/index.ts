import ProfileCredentialsEdit from "./profileCredentialsEdit";
...
export interface ProfileCredentialsProps extends Partial<BaseComponentProps> {
  title?: string;
  subTitle?: string;
  description?: string;
  button?: ButtonConfig;
...
export const defaultProfileCredentialsProps: Required<Omit<ProfileCredentialsProps, 'textArray'>> & { textArray: { title: string; description: string }[] } = {
  ...
  buttonText: "", // Keep for backward compatibility
  array: [],
  images: {
    profile: {
